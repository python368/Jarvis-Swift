...
Relay 负责理解目标、寻找路径并完成任务。

---

### 2. Zero Learning Curve

Relay 应该像普通 Mac App 一样简单。
...
Relay 不依赖一大坨系统提示词来强行规定模型应该成为谁。

默认 System Prompt 应该极其简单，原则上只提供完成 Agent 工作所必须的最低上下文，例如：

> You are Relay. Help the user complete the requested task using the available tools.

具体工具定义、当前系统状态、权限、任务状态和执行边界主要由 Relay Runtime 管理，而不是堆在 System Prompt 里。
...
**用户如果坚持接入一个不适合 Relay 的模型，也可以。**

Relay 不负责替用户判断模型聪不聪明、会不会看图、会不会听音频，或者模型是否适合作为完整 Agent。

可以在必要场景提供一次简单、明确的免责声明：

> 你选择的 AI 模型由你负责。Relay 不保证第三方模型支持视觉、音频、工具调用或其他能力。模型能力不足造成的功能异常，请自行承担。
...
Relay 是纯开源应用，不应围绕模型 API 建立隐藏的中转收费生意。

Relay 不应该：

- 偷偷替用户更换模型
- 把高级模型请求转到廉价模型
- 强制引导用户使用 Relay 自己的模型中转站
- 通过隐藏代理抽取额外费用
- 把模型供应商的能力包装成 Relay 自己的“高级模型”再转售
...
Relay 的角色是 **客户端 + Agent Runtime + macOS 操作层**。

---

## 模型与后端系统

Relay 需要支持丰富的本地和云端 AI 后端，并通过统一抽象层为 Agent 提供一致体验。
...
> **只要服务愿意通过某种接口把模型能力交出来，Relay 就尽量让它能够被接入。**

---

## 搜索系统

搜索是 Relay 的可选增强能力，而不是强制依赖。

没有搜索 Provider 时，Relay 依然应该能够正常完成本地 Agent 工作。
...
Relay 应拥有类似现代 AI 助手的长期记忆能力，同时进一步支持**所有对话之间的共享记忆与历史检索**。
...
Relay 必须是一个真正的原生 macOS App，而不是网页套壳。
...
> **Apple 式材质与空间感 + Google 式几何美感与动态交互 + Relay 自己的视觉人格**
...
这样用户只选择一个颜色，整个 Relay 视觉系统就能自然变化。

> **设计语言由 Relay 定义，视觉个性由用户决定。**
...
Relay 不采用 Macintosh 经典电脑造型、经典像素角色、复古 Mac UI 仿制或其他容易与 Apple 历史品牌视觉产生混淆的元素。

原因不是否定复古设计，而是 Relay 已经确定采用现代原生 macOS 视觉体系，没有必要为了复古彩蛋主动增加品牌相似性和法律风险。

Relay 应建立自己的视觉人格，而不是依赖 Macintosh 视觉符号。

---

## 默认用户体验

Relay 的核心体验应该接近现代 AI 客户端：
...
尤其因为 Relay 能访问屏幕、文件和系统操作权限，隐私不是“以后再加”的功能，而是基础设计要求。
...
Relay 的开源不仅意味着公开源代码，也意味着尽量减少人为锁定。
...
> **Relay 的价值来自工具、执行能力和优秀体验，而不是通过锁死用户来创造收入。**
...
> **模型可以换，Provider 可以换，本地和云端可以换，搜索可以换，记忆可以关，界面可以个性化，但 Relay 的核心体验始终保持一致。**
